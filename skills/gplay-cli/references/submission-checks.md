...
```bash
gplay --dry-run --output table release \
  --package com.example.app \
  --track production \
  --bundle app.aab \
...
echo "=== Step 6: Check mapping file exists ==="
test -s "$MAPPING"
...
- `gplay --dry-run release` creates a real edit session but discards it after validation.
