...
First use `gplay pricing convert` to obtain Google's current converted price and
currency for every target region. Apply these multipliers to each converted
regional price, never directly to the numeric USD amount, then use the currency's
normal precision and valid Play price points. The table expresses the intended
...
```json
{
  ...
  "prices": {
    ...
    "IN": { "priceMicros": "249990000", "currency": "INR" },
