...
Use this skill to explain why a release cannot proceed and to manage an existing review submission. Hand healthy release execution back to [release flow](release-flow.md).
...
Read [submission-readiness-repairs.md](submission-readiness-repairs.md) when diagnostics identify one of those common blockers or a first-release availability gap.

Read [submission-digital-goods.md](submission-digital-goods.md) only when IAP or subscription validation fails, Apple requires first-review attachment, or a versioned product must join an existing review submission.

Read [submission-app-privacy.md](submission-app-privacy.md) only when validation reports an App Privacy advisory or the publish state cannot be confirmed through the public API.

When validation reports a Game Center component or version blocker, hand it to [release flow](release-flow.md) and request the multi-item reference's **Prepare every item** section. Do not route Game Center through general readiness or digital-goods repairs.
...
A version is ready to return to [release flow](release-flow.md) when:
...
- any Game Center version items have been checked through [release flow](release-flow.md)'s multi-item submission reference;
...
5. Hand the healthy version and any preserved `SUBMISSION_ID` to [release flow](release-flow.md) for submission execution. Reuse an inspected `READY_FOR_REVIEW` draft; create a submission only when no matching draft or active submission exists.
...
| Game Center component or version is not ready | `asc validate` diagnostic | [release flow](release-flow.md) multi-item preparation section |
...
- Do not submit from this skill; return healthy execution to [release flow](release-flow.md).
