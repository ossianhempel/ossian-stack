...
Read for requested deployment or PDF export. Follow the scope and safety contract in the skill entry point.
...
If sharing/export is already requested, carry it out within that authority. Otherwise after delivery, **ask the user:** _"Would you like to share this presentation? I can deploy it to a live URL (works on any device including phones) or export it as a PDF."_
...
3. **Deploy** — Run the deploy script:
   ...
   ```bash
   SKILL_DIR="<absolute directory containing the loaded SKILL.md>"; bash "$SKILL_DIR/scripts/deploy.sh" <path-to-presentation>
   ```
...
- **Prefer folder deployments when the presentation has many assets.** If the presentation lives in a folder with images alongside it (e.g., `my-deck/index.html` + `my-deck/logo.png`), deploy the folder directly: `SKILL_DIR="<absolute directory containing the loaded SKILL.md>"; bash "$SKILL_DIR/scripts/deploy.sh" ./my-deck/`. This is more reliable than deploying a single HTML file because the entire folder contents are uploaded as-is.
...
1. **Run the export script:**
   ...
   ```bash
   SKILL_DIR="<absolute directory containing the loaded SKILL.md>"; bash "$SKILL_DIR/scripts/export-pdf.sh" <path-to-html> [output.pdf]
   ```
...
- **Large presentations produce large PDFs.** Each slide is captured as a full 1920×1080 PNG screenshot. An 18-slide deck can produce a ~20MB PDF. If the PDF exceeds 10MB, ask the user: _"The PDF is [size]. Would you like me to compress it? It'll look slightly less sharp but the file will be much smaller."_ If yes, re-run the export with the `--compact` flag:
  ```bash
  SKILL_DIR="<absolute directory containing the loaded SKILL.md>"; bash "$SKILL_DIR/scripts/export-pdf.sh" <path-to-html> [output.pdf] --compact
  ```
