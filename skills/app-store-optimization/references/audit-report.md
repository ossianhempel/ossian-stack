...
Use this structure for every ASO audit. Fill every section; when a field cannot be
observed from the public listing, say so in **Limitations** rather than leaving it
blank.
...
Without paid ASO tools, exact search volume is unavailable. These suggestions come
from category analysis, competitor metadata, and semantic relevance. Validate with
AppTweak, Sensor Tower, or MobileAction for volume data.
...
> - Install conversion rate data (only available to the app owner in console)
> - A/B test results from previous experiments
>
> For these data points, consider AppTweak, Sensor Tower, or MobileAction.
