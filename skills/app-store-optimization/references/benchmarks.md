...
Industry data from AppTweak, SplitMetrics, Sensor Tower, AppFollow, MobileAction,
and Google/Apple documentation. Figures are directional — treat them as orders of
magnitude, not guarantees, and label them as estimates when you cite them.
...
\*Above 100% = some users install from search without visiting the product page.

Source: AppTweak Benchmarks Report (US market).
...
| 0.4-star gap vs competitor | **~25% lost installs** from the same search |
| 3-star vs 5-star app | **50% fewer conversions** for the 3-star app |

**Critical thresholds:**

- **4.0 stars** = minimum for Apple featuring, user trust, and conversion viability
...
**iOS:** +20-40% conversion lift (video autoplays on the product page)
**Google Play:** minimal lift (only ~6% of visitors tap to play)

- Autoplay (introduced iOS 11) caused a **+47% conversion jump**
- Users who watch the video are **2x more likely to install**
- Average watch time: **4-6.5 seconds** (the first 5 seconds are critical)
...
- **The first screenshot decides everything**
...
- **57% of top games** A/B tested screenshots at least twice in 2024
...
- Apple Ads CPP CVR: **55.8%** (up from 42.1% the prior year)
...
- Up to **50 custom versions** per app (100 for Play partners)
...
- **The boost is short-lived** — KPIs drop to baseline when the event ends
