...
**Apply the brand maturity tier adjustments** determined in the audit workflow in `SKILL.md`.

A `0` always means "cannot assess — data unavailable", never "terrible". Do not
average a `0` into the final score; report the dimension as unmeasured.

---

## Brand Maturity Adjustments (apply to all dimensions)

Before scoring, classify the app into one of three tiers.

**Dominant apps (Instagram, Uber, Spotify, WhatsApp, Netflix):**

- Brand-only titles score 8+ (the brand IS the keyword)
- Lifestyle/brand screenshots score the same as captioned UI screenshots
...
- Strategic description/visual choices get the benefit of the doubt
...
data-informed choice by a team with more information than I have?"
...
- Special characters or claims ("#1", "best") that risk rejection (Apple) or are prohibited outright (Google Play)
...
- Screenshot count (minimum 5, ideal 8-10 on Apple, max 8 on Google Play)
- Caption/overlay text on screenshots (one message per screen, 5-7 words max)
- First 3 screenshots (highest conversion impact on Apple — 90% never scroll past the 3rd)
...
- Review recency (fresh reviews signal an active user base)
...
- Related apps from the same developer (portfolio trust signal)
