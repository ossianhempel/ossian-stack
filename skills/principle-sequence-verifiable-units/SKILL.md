...
**Execution.** In a sweep, migration, or any run of similar edits, verify each change before starting the next. Never batch the edits and verify once at the end. Each unit is a before/after bracket: known-good state, one change, run the check, then proceed. Start from the clean baseline required by the project's branching conventions so every check measures the intended state. When a lever does the edits, the per-unit check is nearly free; run it anyway.
