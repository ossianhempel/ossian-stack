...
## Install scopes and auto-update

Verified 2026-09-02 against each runtime's official docs and local behavior:

| Runtime | Global install | Auto-update | Project-scoped install |
| --- | --- | --- | --- |
| Claude Code | Git marketplace | session start (SHA-keyed version-less manifests) | per-project enable/disable via project `.claude/settings.json` |
...
Copilot has a true repo-scoped install surface.
